...
def get_length_of_password(option, default, pass_max_length=PASSWORD_MAX_LENGTH, pass_min_length=PASSWORD_MIN_LENGTH):
    while True:
...
def select_default_or_change_settings(settings):
    while True:
        generate_another_password = input('Do you want to change default generate password?'
                                        ' n = no , y = yes, Enter = yes: ')
        if generate_another_password in ['n', 'y', '']:
            if generate_another_password in ['y', '']:
                get_setting_from_user(settings)
            break
        else:
            print('*** Enter just n , y , Enter key ***')
            print('            Try again ')
...
def check_true_answer_for_another_password():
    while True:
        generate_another_password = input('Do you want another password? n = no , y = yes, Enter = yes: ')
        if generate_another_password in ['n', 'y', '']:
            if generate_another_password == 'n':
                print()
                print('**** The Generate password is finish ****')
                print()
                return False
            else:
                break
...
def generate_another_password_(settings):
    while True:
        print()
        print(f'Generated Password: {password_generator(settings)}', '\n')
        if check_true_answer_for_another_password() == False:
            break
...
def run_prog():
    clear_screen()
    select_default_or_change_settings(settings)
    generate_another_password_(settings)
